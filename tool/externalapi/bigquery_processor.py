...
from dependency.schema import BQ_LOG_DATETIME_FIELD, SCHEMA_AD_ACCOUNT_RAW_TEXTS
    ...
    for i in domain:
        query = f'''
            SELECT DISTINCT 
              * 
            FROM `{table_id}`
            WHERE _TABLE_SUFFIX = '{i}' AND DATE(log_datetime) BETWEEN  '{start_date}' and '{end_date}'; 
        '''
        querys.append(query)
    return querys
